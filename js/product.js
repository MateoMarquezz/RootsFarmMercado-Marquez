...
export class CustomProduct extends Product {
    constructor(name, description, price, customProduct) {
        ...
        this.componentsSelected = []; //Corresponde al stock seleccionado
    }
...
const componentsButtonsListeners = () => {
    ...
    getElementDom('#confirmCustomProduct').addEventListener('click', () => { //Selecciono el <confirmCustomProduct>
        ...
        const customProduct = new CustomProduct("Bolson Personalizado"); //Creo un customProduct para trabajar en caso de haber productos personalizados:

        customProduct.addToOrder(1, inputNotes.value); // <- product.js

        //Cierro la ventana con los componentes (stock)
        $('.stock-modal').fadeOut('fast');
...
//Agrego componentes desde JSON o una Api externa. En este caso JSON
$.ajax({
    type: "GET",
